...
def create_plots(csv_path):
    ...
    ax.set_title('Proposer Anger Behavior')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    fig.savefig(f'proposer_anger_{timestamp}.png')
    plt.close(fig)
    ...
    ax.set_title('Proposer Happiness Behavior')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    fig.savefig(f'proposer_happiness_{timestamp}.png')
    plt.close(fig)
    ...
    ax.set_title('Proposer No Emotion Behavior')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    fig.savefig(f'proposer_no_emotion_{timestamp}.png')
    plt.close(fig)
    ...
    ax.set_xlabel('LLM Model')
    ax.set_ylabel('Value (%)')
    ax.set_title('Proposer Combination Behavior')
    ax.set_xticks(x)
    ax.set_xticklabels(df['llm_abbrev'], rotation=45, ha='right')
    ax.legend()
    plt.tight_layout()
    fig.savefig(f'proposer_combination_{timestamp}.png')
    plt.close(fig)

if __name__ == "__main__":
    csv_path = './proposer_analysis.csv'  # Adjust path if needed
